...
def export_logs(request):
    try:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="xscout_logs_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
...
def system_backup(request):
    try:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="xscout_backup_{datetime.now().strftime("%Y%m%d")}.json"'
        )
        return response
    except Exception as e:
        return JsonResponse({"status": "error", "message": str(e)}, status=500)
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "message": f"Purged {deleted_count} old records.",
                }
            )
        except Exception as e:
            return JsonResponse(
...
def get_directory_structure(request):
    ...
    """
    # Go up 3 levels: views.py -> dashboard -> AdminDashboard -> xScout
    ...
    if not os.path.exists(full_path):
        # DEBUG: Return what we tried to find
        return JsonResponse(
            {
                "status": "error",
                "message": f"Path not found: {target_path} (Base: {base_dir})",
            },
            status=404,
        )
...
def read_file_content(request):
    ...
    # Check if target_path is absolute (from Extension) or relative
    ...
    # Security check:
    # If absolute path provided, we allow it if it exists (Forensic Mode)
    # If relative, we ensure it's inside BASE_DIR
    if not os.path.isabs(target_path) and not os.path.abspath(
        full_path
    ).startswith(base_dir):
        return JsonResponse(
            {"status": "error", "message": "Access denied"}, status=403
    ...
    if not os.path.isfile(full_path):
        return JsonResponse(
            {"status": "error", "message": f"File not found: {full_path}"},
            status=404,
        )
    ...
    if ext.lower() not in allowed_extensions:
        return JsonResponse(
            {
                "status": "error",
                "message": f"File type ({ext}) not supported for viewing",
            },
            status=400,
        )
    ...
    try:
        with open(full_path, "r", encoding="utf-8") as f:
            content = f.read()
        return JsonResponse(
            {"status": "success", "content": content, "path": target_path}
        )
    except UnicodeDecodeError:
        return JsonResponse(
            {
                "status": "error",
                "message": "Binary or non-UTF-8 file cannot be viewed",
            },
            status=400,
        )
    except Exception as e:
...
# --- Environment / Classroom Logic ---


@csrf_exempt
@login_required
def create_environment(request):
    """Generates a new environment with a unique invite code (SQLite)"""
    if request.method == "POST":
        try:
            # Generate 5-char random code
            code = "".join(
            ...
            return JsonResponse({"status": "success", "invite_code": code})
        except Exception as e:
            return JsonResponse(
                {"status": "error", "message": str(e)}, status=500
            )
...
@login_required
def environment_view(request, env_code):
    """Renders the Environment Grid View"""
    return render(request, "environment.html", {"invite_code": env_code})


@login_required
def monitor_view(request, student_id):
    """Renders the Standalone Monitor View for a specific student"""
    return render(request, "monitor.html", {"student_id": student_id})


@login_required
def get_environment_data(request, env_code):
    """API to fetch all students in a specific environment"""
    try:
        # Query users who have this environment tag (We will implement this field in extension next)
        # Note: We query the 'telemetry' collection for users with 'environment' == env_code
        docs = (
            db.collection("telemetry")
            .where("environment", "==", env_code)
            .stream()
        )
...
def get_all_environments(request):
    """API to fetch list of all environments (SQLite)"""
    try:
...
def delete_environment(request):
    ...
    if request.method == "POST":
        try:
            ...
            if not code:
                return JsonResponse(
                    {"status": "error", "message": "Missing code"}, status=400
                )

            Environment.objects.filter(invite_code=code).delete()
...
@csrf_exempt
@login_required
def update_environment_code(request):
    """Updates the invite code for an environment (SQLite)"""
    if request.method == "POST":
        try:
            body = json.loads(request.body)
            ...
            if not old_code or not new_code:
                return JsonResponse(
                    {"status": "error", "message": "Missing codes"}, status=400
                )

            # 1. Check if new code exists
            if Environment.objects.filter(invite_code=new_code).exists():
                return JsonResponse(
                    {"status": "error", "message": "Code already taken"},
                    status=400,
                )

            # 2. Get old env
            try:
                env = Environment.objects.get(invite_code=old_code)
            ...
            # 3. Update code (Since PK, we might need to delete/create or just update depending on DB,
            # but usually PK update is tricky. Easier to just create new and delete old or use update if supported)
            # Actually, standard way is to create new and delete old to be safe with cascading,
            # but since we have no foreign keys pointing to it yet (except maybe logic), we can try atomic update.
            # But changing PK in Django is not direct.

            # Strategy: Create new, Delete old.
            new_env = Environment.objects.create(
                invite_code=new_code,
                created_by=env.created_by,
                description=env.description,
            )
            # Retain `created_at`? created_at is auto_now_add, so it resets.
            # We can manually set it if we remove auto_now_add, but for now reset is okay or we copy it.
            new_env.created_at = env.created_at
            new_env.save()
            ...
            print(f"DEBUG: Error in update_environment_code: {e}")
            return JsonResponse(
                {"status": "error", "message": str(e)}, status=500
            )
    return JsonResponse(
