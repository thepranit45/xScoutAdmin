...
1.  **Source code**: Enter the path to your folder: `/home/thepranit19/xScoutAdmin`.
2.  **Virtualenv**: Enter the path to your env: `/home/thepranit19/.virtualenvs/myenv`.

## 5. Configure WSGI File
1.  Click the link next to **WSGI configuration file** (it looks like `/var/www/thepranit19_pythonanywhere_com_wsgi.py`).
2.  Delete everything and paste this:
    ...
    path = '/home/thepranit19/xScoutAdmin'
...
2.  Under **Static files**:
    - **URL**: `/static/`
    - **Directory**: `/home/thepranit19/xScoutAdmin/staticfiles` (Note: Run `python manage.py collectstatic` in the console first if this folder doesn't exist).

## 7. Upload Secret Key (Important!)
Since we hid `serviceAccountKey.json` from GitHub for security, you must upload it manually.
1.  Go to the **Files** tab on PythonAnywhere.
2.  Navigate to your project folder: `xScoutAdmin`.
3.  Click the **"Upload a file"** button.
4.  Select `serviceAccountKey.json` from your computer (it is in `D:\xScout\AdminDashboard`).
